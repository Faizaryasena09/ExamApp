...
const WebSettingsPage = () => {
  ...
  const handleDeleteTable = (table) => {
    if (
      !window.confirm(
        `Anda yakin ingin menghapus tabel ${table}? Tindakan ini tidak dapat diurungkan.`
      )
    )
      return;
  ...
  const handleResetDB = () => {
    if (
      !window.confirm(
        "PERHATIAN: Anda yakin ingin mereset seluruh database? Semua data akan hilang secara permanen!"
      )
    )
      return;
  ...
  const handleRestart = () => {
    ...
    api.post("/restart-server").then(() =>
      toast.success("Server sedang direstart...")
    );
  };
  ...
  const logoPreviewSrc = logoFile
    ...
    : toAbsoluteImageSrc(logoFromServer);
      ...
      <div className="space-y-10">
        ...
        <div className="bg-white p-6 shadow-md rounded-lg border border-gray-200">
              ...
              tables.map((table, i) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-red-600 hover:text-red-800 transition-colors duration-150"
                  >
                    <TrashIcon />
          ...
          <div className="mt-6 border-t pt-6">
            ...
            <button
              ...
              className="bg-red-600 text-white px-4 py-2 rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition duration-150 ease-in-out"
            >
              Reset Database
            </button>
          </div>
        ...
        <div className="bg-white p-6 shadow-md rounded-lg border border-gray-200">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Gunakan tombol di bawah ini untuk memulai ulang server aplikasi.
          </p>
          <button
            onClick={handleRestart}
            className="bg-gray-700 text-white px-4 py-2 rounded-md shadow-sm hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition duration-150 ease-in-out"
          >
            Restart Server
          </button>
        </div>
