...
exports.logJawaban = async (req, res) => {
    ...
    try {
      // Cek apakah log aktif
      const [cek] = await db.query(
        "SELECT logPengerjaan FROM courses WHERE id = ?",
        [course_id]
      );
      const logAktif = cek[0]?.logPengerjaan === 1;
      if (!logAktif) return res.status(204).end(); // log tidak aktif
  
      // Cari attempt terakhir di jawaban_siswa, lalu tambahkan 1
      ...
      // Simpan log ke student_work_log
      await db.query(
  ...
  exports.getLogDetail = async (req, res) => {
    ...
    try {
      ...
      const [user] = await db.query(
        `SELECT name, kelas FROM users WHERE username = ? LIMIT 1`,
        [userId]
      );
  
      res.json({ logs, user: user[0] });
    } catch (err) {
